...
class SwinDeepLabV3(Model):
    ...
    def call(self, inputs):
        ...
        # Cast resize outputs to match tensor dtype (BF16-safe)
        x = tf.cast(tf.image.resize(high_feat, tf.shape(low_feat)[1:3]), high_feat.dtype)
        x = tf.concat([x, low_feat], axis=-1)
        x = self.decoder(x)
        x = tf.cast(tf.image.resize(x, self.input_size), x.dtype)
